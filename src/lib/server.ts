...
function parseSizeToBytes(value: unknown, fallback: number): number {
    if (_.isFinite(value)) return Number(value);
    if (!_.isString(value)) return fallback;
...
class Server {
    ...
    constructor() {
        ...
        this.app.use(async (ctx: any, next: any) => {
            if (ctx.is('application/json') && ['POST', 'PUT', 'PATCH'].includes(ctx.method)) {
                ...
                const body = await this.#readRawJsonBody(ctx);
                const cleanedBody = this.#cleanJsonString(body);
                const parsedBody = this.#parseJsonWithRepair(body, cleanedBody);
    ...
    async #readRawJsonBody(ctx: any): Promise<string> {
        const chunks: Buffer[] = [];
        const jsonLimitBytes = parseSizeToBytes(config.system.requestBody?.jsonLimit, 100 * 1024 * 1024);
        let receivedBytes = 0;
        await new Promise((resolve, reject) => {
            ...
            const onData = (chunk: Buffer) => {
                ...
                if (receivedBytes > jsonLimitBytes) {
                    return done(new Error(`JSON body too large: limit ${jsonLimitBytes} bytes`));
                }
                chunks.push(chunk);
            };
            const onEnd = () => done();
    ...
    #cleanJsonString(body: string): string {
        return body
            .replace(/\r\n/g, '\n')
            .replace(/\r/g, '\n')
            .replace(/\u00A0/g, ' ')
    ...
    #parseJsonWithRepair(rawBody: string, cleanedBody: string) {
        try {
            return JSON.parse(cleanedBody);
        } catch (parseError: any) {
            logger.error(`JSON解析失败: ${parseError.message}`);
            logger.debug(`原始请求体: ${rawBody.substring(0, 200)}...`);
